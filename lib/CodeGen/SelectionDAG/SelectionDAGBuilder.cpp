...
// expandMathVectorIntrinsics - return the canonical non-vector libm
// function name for the experimental Intrinsics.
static const char*
expandMathVectorIntrinsics(const llvm::EVT &OpEVT, unsigned Opcode,
                           bool IsFastMath, bool IsFiniteMathOnly) {
  llvm::MVT OpMVT;

  if (OpEVT.isVector())
    OpMVT = OpEVT.getVectorElementType().getSimpleVT();
  else
    OpMVT = OpEVT.getSimpleVT();

  assert(OpMVT.isFloatingPoint() && "Expected a floating-point type!");
  ...
  case ISD::FACOS:
    if (!OpMVT.isVector())
      return OpMVT == MVT::f32 ? "acosf" :
                               OpMVT == MVT::f64 ? "acos" : "acosl";
    return nullptr;
    break;
  case ISD::FASIN:
    if (!OpMVT.isVector())
      return OpMVT == MVT::f32 ? "asinf" :
                               OpMVT == MVT::f64 ? "asin" : "asinl";
    return nullptr;
    break;
  case ISD::FATAN:
    if (!OpMVT.isVector())
      return OpMVT == MVT::f32 ? "atanf" :
                               OpMVT == MVT::f64 ? "atan" : "atanl";
    return nullptr;
    break;
  case ISD::FATAN2:
    if (!OpMVT.isVector())
      return OpMVT == MVT::f32 ? "atan2f" :
                               OpMVT == MVT::f64 ? "atan2" : "atan2l";
    return nullptr;
    break;
  case ISD::FTAN:
    if (!OpMVT.isVector())
      return OpMVT == MVT::f32 ? "tanf" :
                               OpMVT == MVT::f64 ? "tan" : "tanl";
    return nullptr;
    break;
  case ISD::FCOSH:
    if (!OpMVT.isVector())
      return OpMVT == MVT::f32 ? "coshf" :
                               OpMVT == MVT::f64 ? "cosh" : "coshl";
    return nullptr;
    break;
  case ISD::FSINH:
    if (!OpMVT.isVector())
      return OpMVT == MVT::f32 ? "sinhf" :
                               OpMVT == MVT::f64 ? "sinh" : "sinhl";
    return nullptr;
    break;
  case ISD::FTANH:
    if (!OpMVT.isVector())
      return OpMVT == MVT::f32 ? "tanhf" :
                               OpMVT == MVT::f64 ? "tanh" : "tanhl";
    return nullptr;
    break;
  case ISD::FACOSH:
    if (!OpMVT.isVector())
      return OpMVT == MVT::f32 ? "acoshf" :
                               OpMVT == MVT::f64 ? "acosh" : "acoshl";
    return nullptr;
    break;
  case ISD::FASINH:
    if (!OpMVT.isVector())
      return OpMVT == MVT::f32 ? "asinhf" :
                               OpMVT == MVT::f64 ? "asinh" : "asinhl";
    return nullptr;
    break;
  case ISD::FATANH:
    if (!OpMVT.isVector())
      return OpMVT == MVT::f32 ? "atanhf" :
                               OpMVT == MVT::f64 ? "atanh" : "atanhl";
    return nullptr;
    break;
  case ISD::FLGAMMA:
    if (!OpMVT.isVector()) {
  ...
  case ISD::FTGAMMA:
    if (!OpMVT.isVector())
      return OpMVT == MVT::f32 ? "tgammaf" :
                               OpMVT == MVT::f64 ? "tgamma" : "tgammal";
    return nullptr;
    break;
  case ISD::FEXP10:
    if (!OpMVT.isVector())
      return OpMVT == MVT::f32 ? "exp10f" :
                               OpMVT == MVT::f64 ? "exp10" : "exp10l";
    return nullptr;
    break;
  }
...
SelectionDAGBuilder::visitIntrinsicCall(const CallInst &I, unsigned Intrinsic) {
  ...
  case Intrinsic::exp10: {
    ...
    case Intrinsic::lgamma:    Opcode = ISD::FLGAMMA;    break;
    case Intrinsic::tgamma:    Opcode = ISD::FTGAMMA;    break;
    ...
    return expandMathVectorIntrinsics(OpEVT, Opcode, IsFastMath,
                                      IsFiniteMathOnly);
  }
  case Intrinsic::sqrt:
  case Intrinsic::fabs:
